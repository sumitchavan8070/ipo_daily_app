...
import 'package:ipodaily/utilities/common/scaffold_messenger.dart';
...
class AuthController extends GetxController with StateMixin<UserModel> {
  ...
  googleSignIn({bool? isPop = false, required CallApiType type}) async {
    ...
    try {
      ...
      logger.e("AuthController => Error during Google sign-in: $e");
      messageScaffold(
  ...
  Future<void> saveGoogleUserToFirestore(GoogleSignInAccount googleUser) async {
    try {
      final userRef = _firestore.collection('client_data').doc(googleUser.id);
  ...
  Future<void> googleSignOut(BuildContext context) async {
    ...
    try {
      ...
      logger.e("AuthController => Error during Google sign-out: $e");
      messageScaffold(
        content: "Something Went Wrong!",
        messageScaffoldType: MessageScaffoldType.error,
      );
  ...
  Future<void> fetchUserData(String uid) async {
    try {
      DocumentSnapshot doc = await _firestore.collection('client_data').doc(uid).get();
      if (doc.exists) {
